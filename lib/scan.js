...
async function scan(opts = {}) {
  ...
  if (opts.online) {
    try {
      ...
      osv = [];
      for (const [nv, ids] of map) {
        const [name, version] = splitNV(nv);
        // attach as findings too
        findings.push({ severity: 'HIGH', sevNum: SEV.HIGH, code: 'osv-advisory', message: `OSV advisories: ${ids.join(', ')}`, evidence: 'https://osv.dev/' + ids[0], package: name, version, store: 'multiple', dir: null });
        osv.push({ package: name, version, advisories: ids });
      }
    } catch (e) {
      osv = { error: e.message };
    }
