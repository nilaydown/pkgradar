...
function checkWormFiles(pkg) {
  ...
  while (stack.length && budget-- > 0) {
    ...
    for (const e of ents) {
      ...
      if (low === '.github' || (d.endsWith(path.sep + '.github') && low.includes('workflow'))) {
        out.push({ sev: SEV.HIGH, code: 'embedded-gh-workflow', msg: `package ships a GitHub Actions workflow (${e.name}) — abnormal for a dependency`, evidence: path.relative(pkg.dir, full) });
      }
    }
  }
  return out;
...
function checkObfuscation(pkg) {
  ...
  while (stack.length && budget-- > 0) {
    ...
    for (const e of ents) {
      if (e.name === 'node_modules' || e.name === 'test' || e.name === 'tests' || e.name === '__tests__') continue;
      const full = path.join(d, e.name);
      if (e.isDirectory()) { stack.push(full); continue; }
      if (/\.(c|m)?js$/.test(e.name)) files.push(full);
      if (files.length > 400) break;
    }
  }
  for (const f of files) {
    const r = readSafe(f, 6_000_000);
    if (!r) continue;
    const t = r.text;
    ...
    if (sev != null) {
      out.push({ sev, code: 'suspicious-js', msg: reasons.join('; '), evidence: path.relative(pkg.dir, f) + (r.truncated ? ` (${(r.size / 1e6).toFixed(1)}MB, scanned first 6MB)` : '') });
    }
  }
  return out;
}
